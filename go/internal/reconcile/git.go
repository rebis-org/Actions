...
func newGit(
	...
	for _, secret := range [...]string{password, credential, bypass.Token} {
		if secret != "" {
...
func platformMapping(remoteName string) pathMapping {
	...
	case PlatformCodeberg, PlatformForgejo:
...
func (repo gitRepository) filter(
	...
	if repo.source.force {
		commit, err := repo.stagedChanges(ctx)

		return branchSnapshot{commit: commit, push: true}, err
	}

	return repo.filteredSnapshot(ctx, remoteBranch)
}

func (repo gitRepository) filteredSnapshot(
	ctx context.Context,
	remoteBranch string,
) (branchSnapshot, error) {
	...
	push := !exists
	if exists {
		identical, err := repo.matchesReference(ctx, remoteBranch)
		if err != nil {
			return branchSnapshot{}, err
		}
	...
	return branchSnapshot{commit: commit, push: push}, nil
}
...
func (repo gitRepository) stagedChanges(ctx context.Context) (bool, error) {
	output, err := repo.output(ctx, "diff", "--cached", "--name-only")

	return strings.TrimSpace(output) != "", err
}

func (repo gitRepository) matchesReference(
	ctx context.Context,
	reference string,
) (bool, error) {
	output, err := repo.output(ctx, "diff", "--cached", "--name-only", reference)
...
func (git Git) environment(endpoint string) []string {
	...
	if git.bypass.matches(endpoint) {
		environment = append(
			environment,
			fmt.Sprintf("GIT_CONFIG_KEY_%d=http.%s.extraHeader", keys, endpoint),
			fmt.Sprintf("GIT_CONFIG_VALUE_%d=%s: %s", keys, bypassHeader, git.bypass.Token),
		)
		keys++
	}

	return append(environment, fmt.Sprintf("GIT_CONFIG_COUNT=%d", keys))
}
